...
# 音乐拉流子进程（当启用 music_player 时自动启动 serve_music）
_music_server_process = None
...
async def pipe_websocket_to_process(websocket, process, target):
    ...
    try:
        while True:
            ...
            # Skip pure-empty or whitespace-only messages (heartbeat)
            if message.strip() == "":
                logger.debug(f"[{target}] << (skipping empty heartbeat)")
                continue
            ...
            try:
                ...
                logger.error(f"[{target}] Failed to write to process stdin: {exc}")
                raise
    except Exception as e:
...
def _start_music_stream_server():
    """若配置里启用了 music_player，在后台启动 serve_music.py（8080），供 stream_url 拉流。"""
    global _music_server_process
    script_dir = os.path.dirname(os.path.abspath(__file__))
    serve_script = os.path.join(script_dir, "serve_music.py")
    if not os.path.isfile(serve_script):
        return
    cfg = load_config()
    servers = cfg.get("mcpServers") or {}
    entry = servers.get("music_player")
    if not entry or entry.get("disabled"):
        return
    if _is_port_in_use(8080):
        logger.info("[music] 端口 8080 已被占用，音乐拉流服务可能已在运行，stream_url 可用")
        return
    try:
        _music_server_process = subprocess.Popen(
            [sys.executable, serve_script, "8080"],
            cwd=script_dir,
            stdin=subprocess.DEVNULL,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
        logger.info("[music] 音乐拉流服务已启动 (端口 8080)，stream_url 可用")
    except Exception as e:
        logger.warning(f"[music] 启动音乐拉流服务失败: {e}")

def _stop_music_stream_server():
    """停止音乐拉流子进程。"""
    global _music_server_process
    if _music_server_process is not None:
        try:
            _music_server_process.terminate()
            _music_server_process.wait(timeout=3)
        except Exception:
            try:
                _music_server_process.kill()
            except Exception:
                pass
        _music_server_process = None
        logger.info("[music] 音乐拉流服务已停止")

def signal_handler(sig, frame):
    """Handle interrupt signals"""
    _stop_music_stream_server()
    ...
    async def _main():
        if not target_arg:
            ...
            if "music_player" in enabled:
                _start_music_stream_server()
            logger.info(f"Starting servers: {', '.join(enabled)}")
            atexit.register(_stop_music_stream_server)
            ...
            cfg = load_config()
            need_music = (
                target_arg == "music_player"
                or (target_arg and os.path.basename(target_arg) == "mcp_music_player.py")
            )
            if need_music:
                _start_music_stream_server()
            atexit.register(_stop_music_stream_server)
            ...
            elif target_arg in (cfg.get("mcpServers") or {}):
                await connect_with_retry(endpoint_url, target_arg)
            else:
                logger.error("Argument must be a local Python script path or a configured server name. To run all servers, run without arguments.")
                sys.exit(1)
    try:
        asyncio.run(_main())
